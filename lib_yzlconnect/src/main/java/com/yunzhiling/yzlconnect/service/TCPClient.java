...
public class TCPClient {
    ...
    public static void tcpConnect(final int port, String ssid, String password, double lng, double lat, ConnectCallback connectCallback) {
        Log.d("message", "yzlconnect----------->tcpConnect");
        new Thread(() -> {
        ...
        if (checkRunnable == null) {
            checkRunnable = () -> {
                Log.d("message", "yzlconnect----------->tcp connect 15 second");
                if (!isConnected) {
                    Log.d("message", "yzlconnect----------->tcp timeout");
    ...
    public static void stocketConnect(ConnectCallback connectCallback) {
        try {
            ...
            Log.d("message", "yzlconnect----------->tcp stocket connect");
            //连接成功后开始发送和接收
            ...
            Log.d("message", "yzlconnect----------->tcp stocket connect error");
            if (isRunning) {
                if (stocketConnectTimes <= 3) {
                    ...
                    Log.d("message", "yzlconnect----------->tcp stocket connect again times:" + stocketConnectTimes);
                    stocketConnectTimes++;
                    stocketConnect(connectCallback);
                } else {
                    Log.d("message", "yzlconnect----------->tcp stocket connect fail");
                    close();
    ...
    public static void sendOkMessage(String message, int sendTimes, ConnectCallback connectCallback) {
        try {
            ...
            while (socket != null && outputStream != null && currentSendTimes < sendTimes) {
                try {
                    Log.d("message", "yzlconnect----------->tcp send ok times:" + currentSendTimes);
            ...
            Log.d("message", "yzlconnect----------->tcp send ok error");
            close();
            if (connectCallback != null) connectCallback.fail();
        }
    ...
    public static void sendConnectMessage(String message, ConnectCallback connectCallback) {
        try {
            ...
            while (isRunning && !isConnected && outputStream != null && socket != null && !socket.isClosed() && currentSendTimes < 1 && !isReceiveZore) {
                Log.d("message", "yzlconnect----------->tcp send connect data times:" + currentSendTimes);
                try {
                    outputStream.write(message.getBytes());
                    outputStream.flush();
                } catch (Exception e) {
    ...
    public static void receive(ConnectCallback connectCallback) {
        Log.d("message", "yzlconnect----------->tcp receive");
        try {
            ...
            while (isRunning && socket != null && !socket.isClosed() && inputStream != null && ((length = inputStream.read(buffer)) != -1)) {
                ...
                try {
                    ...
                    Log.d("message", "yzlconnect----------->tcp read json error");
                    code = "";
                }
                if (code != null) {
                    Log.d("message", "yzlconnect----------->tcp receive code:" + code);
                }
            ...
            e.printStackTrace();
            Log.d("message", "yzlconnect----------->tcp receive error");
        }
    }

    public static void close() {
        Log.d("message", "yzlconnect----------->tcp close");
        isConnected = false;
        isRunning = false;
        try {
